...
import (
	...
	"ue/src/consumerTestdata/UDR/TestRegistrationProcedure"
...
func (ue *RanUeContext) DeriveRESstarAndSetKey(
	...
	sqnXorAk, err := hex.DecodeString(authSubs.SequenceNumber)
	if err != nil {
...
func (ue *RanUeContext) DeriveResEAPMessageAndSetKey(
	...
	fmt.Println("DeriveResEAP: ", Kseaf)
	if err != nil {
		fatal.Fatalf("GetKDFValue error: %+v", err)
	}
...
func (ue *RanUeContext) DerivateAlgKey() {
	...
	fmt.Println("kamf: ", ue.Kamf)
	fmt.Println("KnasInt: ", ue.KnasInt)
	fmt.Println("KnasEnc: ", ue.KnasEnc)
}
