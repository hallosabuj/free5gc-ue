...
import (
	...
	"ue/src/consumerTestdata/UDM/TestGenAuthData"
	"ue/src/nasTestpacket"
...
func setupGreTunnel(greIfaceName, parentIfaceName string, ueTunnelAddr, n3iwfTunnelAddr, pduAddr net.IP, qoSInfo *PDUQoSInfo) (netlink.Link, error) {
	var (
		parent      netlink.Link
		greKeyField uint32
		err         error
...
func setupUDPSocket() (*net.UDPConn, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Resolve UDP address failed: %+v\n", err)
	}
...
func applyXFRMRule(ue_is_initiator bool, ifId uint32, childSecurityAssociation *context.ChildSecurityAssociation) error {
	...
	if err = netlink.XfrmStateAdd(xfrmState); err != nil {
		return fmt.Errorf("Set XFRM state rule failed: %+v\n", err)
	}
...
func sendPduSessionEstablishmentRequest(
	...
	fmt.Println("Waiting for N3IWF reply from IKE")
	...
	fmt.Printf("IKE message exchange type: %d\n", ikeMessage.ExchangeType)
	fmt.Printf("IKE message ID: %d\n", ikeMessage.MessageID)
	...
	for _, ikePayload := range decryptedIKEPayload {
		...
		case message.TypeN:
			...
			if notification.NotifyMessageType == message.Vendor3GPPNotifyType5G_QOS_INFO {
				fmt.Println("Received Qos Flow settings")
				if info, err := parse5GQoSInfoNotify(notification); err == nil {
					qoSInfo = info
					fmt.Printf("NotificationData:%+v\n", notification.NotificationData)
					if qoSInfo.isDSCPSpecified {
						fmt.Println("DSCP is specified but test not support")
					}
				} else {
					fmt.Printf("%+v\n", err)
				}
	...
	if err := encryptProcedure(ikeSA, ikePayload, ikeMessage); err != nil {
		fmt.Printf("Encrypt IKE message failed: %+v\n", err)
		return ifaces, err
	}
	...
	if err != nil {
		fmt.Printf("Write IKE maessage fail: %+v\n", err)
		return ifaces, err
	}
	...
	// ====== Inbound ======
	fmt.Printf("====== IPSec/Child SA for 3GPP UP Inbound =====")
	fmt.Printf("[UE:%+v] <- [N3IWF:%+v]\n",
		childSecurityAssociationContextUserPlane.LocalPublicIPAddr, childSecurityAssociationContextUserPlane.PeerPublicIPAddr)
	fmt.Printf("IPSec SPI: 0x%016x\n", childSecurityAssociationContextUserPlane.InboundSPI)
	fmt.Printf("IPSec Encryption Algorithm: %d\n", childSecurityAssociationContextUserPlane.EncryptionAlgorithm)
	fmt.Printf("IPSec Encryption Key: 0x%x\n", childSecurityAssociationContextUserPlane.InitiatorToResponderEncryptionKey)
	fmt.Printf("IPSec Integrity  Algorithm: %d\n", childSecurityAssociationContextUserPlane.IntegrityAlgorithm)
	fmt.Printf("IPSec Integrity  Key: 0x%x\n", childSecurityAssociationContextUserPlane.InitiatorToResponderIntegrityKey)
	// ====== Outbound ======
	fmt.Println("====== IPSec/Child SA for 3GPP UP Outbound =====")
	fmt.Printf("[UE:%+v] -> [N3IWF:%+v]\n",
		childSecurityAssociationContextUserPlane.LocalPublicIPAddr, childSecurityAssociationContextUserPlane.PeerPublicIPAddr)
	fmt.Printf("IPSec SPI: 0x%016x\n", childSecurityAssociationContextUserPlane.OutboundSPI)
	fmt.Printf("IPSec Encryption Algorithm: %d\n", childSecurityAssociationContextUserPlane.EncryptionAlgorithm)
	...
	fmt.Printf("State function: encr: %d, auth: %d\n", childSecurityAssociationContextUserPlane.EncryptionAlgorithm, childSecurityAssociationContextUserPlane.IntegrityAlgorithm)
	...
	if err != nil {
		fmt.Printf("Applying XFRM rules failed: %+v\n", err)
		return ifaces, err
	}
	...
	fmt.Printf("Setup XFRM interface %s successfully\n", newXfrmiName)
...
func TestNon3GPPUE() {
	...
	n3iwfUDPAddr, err := net.ResolveUDPAddr("udp", n3iwfInfo_IPSecIfaceAddr+":500")
	if err != nil {
		fmt.Printf("Resolve UDP address %s fail: %+v\n", n3iwfInfo_IPSecIfaceAddr+":500", err)
	}
	udpConnection, err := setupUDPSocket()

	if err != nil {
		fmt.Printf("Setup UDP socket Fail: %+v\n", err)
	}
	...
	if !ok {
		fmt.Println("Generate key exchange data failed")
	}
	...
	ikeMessageData, err := ikeMessage.Encode()
	if err != nil {
		fmt.Printf("Encode IKE Message fail: %+v", err)
	}
	if _, err := udpConnection.WriteToUDP(ikeMessageData, n3iwfUDPAddr); err != nil {
		fmt.Printf("Write IKE maessage fail: %+v", err)
	}
	...
	if err != nil {
		fmt.Printf("Read IKE Message fail: %+v", err)
	}
	ikeMessage.Payloads.Reset()
	err = ikeMessage.Decode(buffer[:n])
	if err != nil {
		fmt.Printf("Decode IKE Message fail: %+v", err)
	}
	...
	if err := generateKeyForIKESA(ikeSecurityAssociation); err != nil {
		fmt.Printf("Generate key for IKE SA failed: %+v\n", err)
	}
	...
	if err := encryptProcedure(ikeSecurityAssociation, ikePayload, ikeMessage); err != nil {
		fmt.Printf("Encrypting IKE message failed: %+v\n", err)
	}

	// Send to N3IWF
	ikeMessageData, err = ikeMessage.Encode()
	if err != nil {
		fmt.Printf("Encode IKE message failed: %+v", err)
	}
	if _, err := udpConnection.WriteToUDP(ikeMessageData, n3iwfUDPAddr); err != nil {
		fmt.Printf("Write IKE message failed: %+v\n", err)
	}
	...
	n, _, err = udpConnection.ReadFromUDP(buffer)
	if err != nil {
		fmt.Printf("Read IKE message failed: %+v", err)
	}
	ikeMessage.Payloads.Reset()
	err = ikeMessage.Decode(buffer[:n])
	if err != nil {
		fmt.Printf("Decode IKE message failed: %+v\n", err)
	}

	encryptedPayload, ok := ikeMessage.Payloads[0].(*message.Encrypted)
	if !ok {
		fmt.Println("Received payload is not an encrypted payload")
	}

	decryptedIKEPayload, err := decryptProcedure(ikeSecurityAssociation, ikeMessage, encryptedPayload)
	if err != nil {
		fmt.Printf("Decrypt IKE message failed: %+v", err)
	}
	...
	ikeMessageData, err = ikeMessage.Encode()
	if err != nil {
		fmt.Printf("Encode IKE message failed: %+v\n", err)
	}
	if _, err := udpConnection.WriteToUDP(ikeMessageData, n3iwfUDPAddr); err != nil {
		fmt.Printf("Write IKE message failed: %+v\n", err)
	}
	...
	n, _, err = udpConnection.ReadFromUDP(buffer)
	if err != nil {
		fmt.Printf("Read IKE message failed: %+v\n", err)
	}
	...
	decryptedIKEPayload, err = decryptProcedure(ikeSecurityAssociation, ikeMessage, encryptedPayload)
	if err != nil {
		fmt.Printf("Decrypt IKE message failed: %+v\n", err)
	}
	...
	if !ok {
		fmt.Println("Received packet is not an EAP payload")
	}
	...
	if !ok {
		fmt.Println("The EAP data is not an EAP expended.")
	}
	...
	if err := decodedNAS.PlainNasDecode(&nasData); err != nil {
		fmt.Printf("Decode plain NAS fail: %+v\n", err)
	}

	// Calculate for RES*
	// assert.NotNil(t, decodedNAS)
	byteSQN := make([]byte, 6)
	copy(byteSQN[:], decodedNAS.AuthenticationRequest.AuthenticationParameterAUTN.Octet[:6])
	fmt.Println("Packet => SQN xor AK : ", byteSQN)
	fmt.Println("Packet => SQN xor AK : ", hex.EncodeToString(byteSQN))
	fmt.Println("Packet => AMAF : ", decodedNAS.AuthenticationRequest.AuthenticationParameterAUTN.Octet[6:8])
	fmt.Println("Packet => MAC : ", decodedNAS.AuthenticationRequest.AuthenticationParameterAUTN.Octet[8:])
	rand := decodedNAS.AuthenticationRequest.GetRANDValue()
	ue.AuthenticationSubs.SequenceNumber = hex.EncodeToString(byteSQN)
	...
	ikeMessageData, err = ikeMessage.Encode()
	if err != nil {
		fmt.Printf("Encode IKE Message fail: %+v\n", err)
	}
	...
	n, _, err = udpConnection.ReadFromUDP(buffer)
	if err != nil {
		fmt.Printf("Read IKE Message fail: %+v\n", err)
	}
	ikeMessage.Payloads.Reset()
	err = ikeMessage.Decode(buffer[:n])
	if err != nil {
		fmt.Printf("Decode IKE Message fail: %+v\n", err)
	}
	encryptedPayload, ok = ikeMessage.Payloads[0].(*message.Encrypted)
	if !ok {
		fmt.Println("Received pakcet is not an encrypted payload")
	}
	decryptedIKEPayload, err = decryptProcedure(ikeSecurityAssociation, ikeMessage, encryptedPayload)
	if err != nil {
		fmt.Printf("Decrypt IKE message failed: %+v\n", err)
	...
	eapExpanded, ok = eapReq.EAPTypeData[0].(*message.EAPExpanded)
	if !ok {
		fmt.Println("Received packet is not an EAP expended payload")
		return
	}
	...
	if err != nil {
		fmt.Printf("Get Kn3iwf error : %+v\n", err)
	}
	Kn3iwf = []byte([]uint8{145, 218, 253, 42, 180, 215, 124, 122, 26, 24, 159, 43, 115, 223, 21, 42, 2, 230, 76, 172, 144, 217, 57, 248, 77, 254, 71, 45, 130, 131, 61, 50})
	fmt.Println("Kn3iwf=================:", Kn3iwf)
	...
	if !ok {
		fmt.Println("Get an unsupported pseudorandom funcion. This may imply an unsupported transform is chosen.")
	}
	...
	if err != nil {
		fmt.Printf("Encode IKE payload failed : %+v\n", err)
	}
	if _, err := pseudorandomFunction.Write(idPayloadData[4:]); err != nil {
		fmt.Printf("Pseudorandom function write error: %+v\n", err)
	}
	...
	for _, ikePayload := range decryptedIKEPayload {
		switch ikePayload.Type() {
		case message.TypeAUTH:
			fmt.Println("Get Authentication from N3IWF")
		case message.TypeSA:
			responseSecurityAssociation = ikePayload.(*message.SecurityAssociation)
	...
	if err != nil {
		fmt.Printf("Create child security association context failed: %+v\n", err)
	}
	...
	if err != nil {
		fmt.Printf("Parse IP address to child security association failed: %+v\n", err)
	}
	// Select TCP traffic
	...
	if err := generateKeyForChildSA(ikeSecurityAssociation, childSecurityAssociationContext); err != nil {
		fmt.Printf("Generate key for child SA failed: %+v\n", err)
	}
	...
	if linkIPSec, err = setupIPsecXfrmi(newXfrmiName, n3ueInfo_IPSecIfaceName, n3ueInfo_XfrmiId, ueInnerAddr); err != nil {
		fmt.Printf("Setup XFRM interface %s fail: %+v\n", newXfrmiName, err)
	}

	defer func() {
		if err := netlink.LinkDel(linkIPSec); err != nil {
			fmt.Printf("Delete XFRM interface %s fail: %+v\n", newXfrmiName, err)
		} else {
			fmt.Printf("Delete XFRM interface: %s\n", newXfrmiName)
		}
	}()
	...
	if err != nil {
		fmt.Printf("NAS Decode Fail: %+v\n", err)
	}

	spew.Config.Indent = "\t"
	nasStr := spew.Sdump(nasMsg)
	fmt.Printf("Get NAS Security Mode Command Message:\n %+v\n", nasStr)
		...
		case message.TypeN:
			...
			if notification.NotifyMessageType == message.Vendor3GPPNotifyType5G_QOS_INFO {
				...
				if info, err := parse5GQoSInfoNotify(notification); err == nil {
					...
					fmt.Printf("NotificationData:%+v\n\n", notification.NotificationData)
					if QoSInfo.isDSCPSpecified {
						fmt.Printf("DSCP is specified but test not support")
					}
				} else {
					fmt.Printf("%+v\n", err)
				}
	...
	if linkGRE, err = setupGreTunnel(newGREName, newXfrmiName, ueInnerAddr.IP, upIPAddr, pduAddress, QoSInfo); err != nil {
		fmt.Printf("Setup GRE tunnel %s Fail %+v\n", newGREName, err)
	}
